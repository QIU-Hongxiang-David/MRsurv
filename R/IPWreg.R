...
#' @param tvals see \code{\link{MRsurv}}. Must be sorted in ascending order and all greater than the smallest time in `pred_censor_obj`
            ...
            if(tvals[i]<=visit.times[k]){ #no regression for this t
                ...
                if(ncol(X)==0){ #intercept-only model
                    if(is.null(obsWeights)){
                        ...
                        models<-c(models,list(intercept_model(mean(Y*obsWeights))))
                    }
                }else{
                    SuperLearner.arg<-c(
