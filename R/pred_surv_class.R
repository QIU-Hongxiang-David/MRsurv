...
#' @param surv matrix of predicted/fitted survival probabilities. Each row corresponds to an individual; each column corresponds to an event time in `time`. Row names should be `id.var` that identifies each individual (see \code{\link{MRsurv}}).
...
#' @description Truncate the time and survival probabilities in a `pred_surv` object to minimal so that predicted/fitted survival probability at `end` is kept. This function can be used before applying transformations (e.g., \code{\link{DRtransform}}, \code{\link{Gtransform}}, \code{\link{IPCWtransform}}) to avoid unnecessary computation. No truncation is applied if `end` is greater than or equal to the last time in the `pred_surv` object.
