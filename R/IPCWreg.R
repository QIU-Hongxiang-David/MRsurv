...
IPCWreg.SuperLearner<-function(
    ...
    X<-model.frame(Q.formula,history%>%filter(.data[[id.var]] %in% names(Y))%>%arrange(.data[[id.var]])%>%select(!all_of(id.var)))
    
    if(is.null(obs.weight.var)){
        obsWeights<-NULL
    }else{
        obsWeights<-follow.up.time%>%filter(.data[[id.var]] %in% names(Y))%>%arrange(.data[[id.var]])%>%pull(all_of(obs.weight.var))
