...
fit_surv<-function(method=c("survSuperLearner","rfsrc","ctree","rpart","cforest","coxph","coxtime","deepsurv","survival_forest","no_event"),...){
...
fit_survSuperLearner<-function(formula,data,id.var,time.var,event.var,nfold=2,time.grid.size=250,option=list(event.SL.library=c("survSL.coxph","survSL.weibreg","survSL.gam","survSL.rfsrc"),cens.SL.library=c("survSL.coxph","survSL.weibreg","survSL.gam","survSL.rfsrc")),obs.weight.var,...){
    ...
    if(any(c("time","event","X","newX","obsWeights") %in% names(option))){
        stop("option should not specify time, event, X, newX, or obsWeights")
    }
    ...
    if(nfold==1){
        ...
        surv.list<-lapply(folds,function(fold){
            ...
            if(d%>%pull(.data[[event.var]])%>%{all(.==0)}){
                ...
                model<-do.call(randomForestSRC::rfsrc,arg)
                predict.model<-predict(model,test.d)
    ...
    if(nfold==1){
        ...
        surv.list<-lapply(folds,function(fold){
            ...
            if(d%>%pull(.data[[event.var]])%>%{all(.==0)}){
                ...
                model<-do.call(party::ctree,arg)
                surv<-lapply(party::treeresponse(model,newdata=test.d),function(surv_fit){
                    sapply(all.times,function(t){
                        i<-find.first.TRUE.index(surv_fit$time<=t,noTRUE=0)
                        if(i==0){
                            out<-1
    ...
    if(nfold==1){
        ...
        surv.list<-lapply(folds,function(fold){
            ...
            if(d%>%pull(.data[[event.var]])%>%{all(.==0)}){
                ...
                model<-partykit::as.party(do.call(rpart::rpart,arg))
                surv<-lapply(predict(model,newdata=test.d,type="prob"),function(surv_fit){
                    sapply(all.times,function(t){
                        i<-find.first.TRUE.index(surv_fit$time<=t,noTRUE=0)
                        if(i==0){
                            out<-1
    ...
    if(nfold==1){
        ...
        surv.list<-lapply(folds,function(fold){
            ...
            if(d%>%pull(.data[[event.var]])%>%{all(.==0)}){
                ...
                cox.model<-do.call(survival::coxph,arg)
                surv<-lapply(all.times,function(t){
                    predict(cox.model,newdata=test.d%>%mutate("{time.var}":=t),type="survival")
                })%>%do.call(what=cbind)
                # model<-survival::survfit(cox.model,newdata=data%>%filter(.data[[id.var]] %in% .env$fold))
                # model.surv<-t(as_matrix_rowvec(model$surv))
                # surv<-lapply(all.times,function(t){
